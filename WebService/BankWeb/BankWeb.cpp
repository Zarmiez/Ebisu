#include "stdafx.h"  //________________________________________ BankWeb.cpp
#include "BankWeb.h"
#include "Login.h"
#include "NewClient.h"
#include "Deposito.h"
...
BOOL WINAPI GetExtensionVersion(HSE_VERSION_INFO* pVer)
{
	...
	strcpy_s(pVer->lpszExtensionDesc, HSE_MAX_EXT_DLL_NAME_LEN, "BankWeb ISAPI Extension");
...
void SelectWindow(const wchar_t* windowID, Web::HttpConnector& h)
{
	...
	else if(wcscmp(windowID, L"Login") == 0) {Login page; page.Run(h);}
	else if(wcscmp(windowID, L"NewClient") == 0) {NewClient page; page.Run(h);}
	else if(wcscmp(windowID, L"Deposito") == 0) {Deposito page; page.Run(h);}
}
